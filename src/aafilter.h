...
public:
	AAFilter();
	AAFilter(const QStringList &artists, const QStringList &albums, bool allAlbums = false);
	bool allAlbums() const;
	bool notTaggedAlbum() const;
	bool hasAlbum(const QString &) const;
	bool hasArtist(const QString &) const;
...
QDataStream &operator>>(QDataStream &, AAFilter &);
#endif
